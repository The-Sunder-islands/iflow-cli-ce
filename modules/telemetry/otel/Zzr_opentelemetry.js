...
var Zzr = T((nLe) => {
  ...
  var ORo = (Zt(), bt(cr)),
    ...
    M4t = class {
      ...
      detect(e) {
        let r = {},
          n = (0, Xzr.getStringFromEnv)("OTEL_RESOURCE_ATTRIBUTES"),
          o = (0, Xzr.getStringFromEnv)("OTEL_SERVICE_NAME");
        if (n)
          try {
            let s = this._parseResourceAttributes(n);
            Object.assign(r, s);
          } catch (s) {
            ORo.diag.debug(`EnvDetector failed: ${s.message}`);
          }
        return (o && (r[NRo.ATTR_SERVICE_NAME] = o), { attributes: r });
      }
